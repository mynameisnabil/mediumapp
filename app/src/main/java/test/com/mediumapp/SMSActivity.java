    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btnsend:
                ...
                //kondisi ketika permission belum diijinkan oleh pengguna (marsmello +)
                if (Build.VERSION.SDK_INT >= 23 &&checkSelfPermission(Manifest.permission.SEND_SMS)
                        != PackageManager.PERMISSION_GRANTED) {
                    //menampilkan konfirmasi pada user untuk mengijinkan mengakses end_sms
                    ...
                    try {
                        ...
                        clearText();
                        Toast.makeText(this,"Mengirim pesan ", Toast.LENGTH_LONG).show();
                    } catch (Exception e) {
                        Toast.makeText(this,"Gagal Mengirim pesan", Toast.LENGTH_LONG).show();
                    }
                }
                break;
            case R.id.btnintent:
